      ...
      <div v-else-if="parsedReport" class="report-body">
        ...
        <div class="section-card">
          ...
          <el-table :data="riskTop5List" size="small" border>
            ...
            <el-table-column prop="target" label="探测目标" min-width="260" show-overflow-tooltip />
            ...
            <el-table-column prop="reason" label="分析说明" min-width="280" show-overflow-tooltip />
          </el-table>
        </div>
        ...
        <div class="section-card">
          ...
          <el-table :data="topTargetsList" size="small" border>
            <el-table-column type="index" width="50" align="center" />
...
const topTargetsList = computed(() => {
  const rows = parsedInput.value?.topTargets;
  if (!Array.isArray(rows)) return [];
  return rows.slice(0, 20);
});
...
const kpiCards = computed(() => {
  ...
  return [
    ...
    { label: "受影响目标", value: topTargetsList.value.length, sub: "探测目标数", color: "#f59e0b" },
    { label: "告警类型", value: alertNameDistList.value.length, sub: "探测规则种类", color: "#8b5cf6" },
    { label: "P1 建议数", value: p1List.value.length, sub: "需立即处理", color: "#06b6d4" }
  ];
});

const issueTagType = (issue: string) => {
  if (issue === "探测失败") return "danger";
  if (issue === "响应慢" || issue === "SSL证书") return "warning";
  return "info";
};
...
const fetchList = () => {
  loading.value = true;
  baseService
    .get("/alert/prometheus/report/latest", { reportType: "http", size: 50 })
    .then((res) => {
      list.value = res.data || [];
    })
    .finally(() => {
...
const generateReport = () => {
  ...
  baseService
    .post(`/alert/prometheus/report/generate?reportType=http&periodType=${form.periodType}`)
    .then((res) => {
      const data = res.data || {};
