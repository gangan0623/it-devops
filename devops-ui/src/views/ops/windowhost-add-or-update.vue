  ...
  <el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
        ...
        <div class="host-instance">
          ...
          <el-input v-model="dataForm.port" placeholder="9182" class="host-instance__port" @blur="handleInstanceBlur"></el-input>
        </div>
      </el-form-item>
...
const getInfo = (id: number) => {
  baseService.get("/ops/windowhost/" + id).then((res) => {
...
const checkUnique = (field: "instance" | "name") => {
  ...
  params[field] = value;
  baseService.get("/ops/windowhost/check", params).then((res) => {
...
const dataFormSubmitHandle = () => {
  ...
  dataFormRef.value.validate((valid: boolean) => {
    ...
    (!dataForm.id ? baseService.post : baseService.put)("/ops/windowhost", submitData).then((res) => {
      ElMessage.success({
        message: '成功',
        duration: 500,
        onClose: () => {
...
const buildInstance = () => {
  ...
  const port = rawPort || "9182";
  return `${ipPart}:${port}`;
};

const parseInstance = (instance: string) => {
  const result = { ip: "", port: "9182" };
  if (!instance) {
    return result;
  }
