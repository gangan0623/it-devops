  ...
  <div class="mod-ops__linuxhost">
    <div class="ops-toolbar">
      <div class="ops-toolbar__row">
        ...
        <div class="ops-toolbar__group ops-actions">
          ...
          <el-button v-if="state.hasPermission('ops:linuxhost:export')" type="info" @click="state.exportHandle()">导出</el-button>
          <el-button v-if="state.hasPermission('ops:linuxhost:import')" type="primary" @click="importDialogVisible = true">导入</el-button>
        </div>
      </div>
    </div>
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" class="ops-table-nowrap" style="width: 100%">
      <el-table-column type="selection" header-align="center" align="center" width="50"></el-table-column>
              ...
              <el-table-column label="站点位置" header-align="center" align="center" min-width="120">
              ...
              <el-table-column label="主机类型" header-align="center" align="center" min-width="100">
        ...
        <template v-slot="scope">
          <el-button v-if="state.hasPermission('ops:linuxhost:update')" type="primary" link @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
          <el-button v-if="state.hasPermission('ops:linuxhost:save')" type="primary" link @click="cloneHandle(scope.row)">克隆</el-button>
          <el-button v-if="state.hasPermission('ops:linuxhost:delete')" type="primary" link @click="state.deleteHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const view = reactive({
  deleteIsBatch: true,
  getDataListURL: "/ops/linuxhost/page",
  getDataListIsPage: true,
  exportURL: "/ops/linuxhost/export",
  deleteURL: "/ops/linuxhost",
  dataForm: {
    instance: "",
    name: "",
    siteLocation: "",
...
const importUrl = `${app.api}/ops/linuxhost/import?token=${getToken()}`;
const templateUrl = `${app.api}/ops/linuxhost/template?token=${getToken()}`;
const uploadHeaders = {
  token: getToken()
};
...
const updateStatusHandle = (status: number) => {
  ...
  ElMessageBox.confirm(`确定进行[${actionLabel}]操作?`, "提示", {
    ...
    baseService.put("/ops/linuxhost/status", { ids, status }).then(() => {
      ElMessage.success({
        message: "成功",
        duration: 500,
        onClose: () => {
