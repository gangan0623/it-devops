    ...
    <el-form :inline="true" :model="state.dataForm" @keyup.enter="state.getDataList()" class="ops-toolbar">
      <div class="ops-toolbar__row">
        ...
        <div class="ops-toolbar__group ops-actions">
          ...
          <el-button v-if="state.hasPermission('ops:businesssystem:update')" type="success" @click="updateStatusHandle(1)">批量启用</el-button>
          <el-button v-if="state.hasPermission('ops:businesssystem:update')" type="warning" @click="updateStatusHandle(0)">批量禁用</el-button>
...
const refreshOnlineStatus = () => {
  ...
  state.dataList.forEach((row: { instance?: string; onlineStatus?: boolean | null }) => {
    ...
    baseService
      .get("/ops/businesssystem/online", { instance: row.instance })
      .then((res) => {
